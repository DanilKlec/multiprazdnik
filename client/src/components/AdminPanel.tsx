...
export default function AdminPanel({
  ...
  <div className="flex flex-col gap-0.5 mt-auto">
    {bList.slice(0, 2).map((booking) => (
      <div
        key={booking.id}
        className="text-[8px] px-1 py-0.5 rounded bg-black text-white truncate"
      >
        {booking.eventTime} {booking.childName}
      </div>
    ))}

    {bList.length > 2 && (
      <div className="text-[8px] text-center font-bold text-amber-600">
        +{bList.length - 2}
      </div>
    )}
  </div>
  ...
  {b.status === 'confirmed' && (
    <button
      ...
      className="px-2 py-1 bg-blue-500 text-white rounded text-[9px] font-bold"
    >
      Выполнено
    </button>
  )}
